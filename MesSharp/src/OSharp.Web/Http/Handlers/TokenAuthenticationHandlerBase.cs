...
namespace Mes.Web.Http.Handlers
{
    public abstract class TokenAuthenticationHandlerBase : DelegatingHandler
    {
        ...
        {
            if (!request.Headers.Contains(HttpHeaderNames.MesAuthenticationToken))
            {
                return base.SendAsync(request, cancellationToken);
            }
            string authenticationToken = request.Headers.GetValues(HttpHeaderNames.MesAuthenticationToken).First();
