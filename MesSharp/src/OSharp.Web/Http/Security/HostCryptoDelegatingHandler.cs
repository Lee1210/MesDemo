...
using Mes.Web.Properties;


namespace Mes.Web.Http.Security
{
    /// <summary>
    /// 服务端通信加密解密消息处理器
    /// </summary>
    public class HostCryptoDelegatingHandler : DelegatingHandler
    {
        ...
        {
            if (!request.Headers.Contains(HttpHeaderNames.MesClientPublicKey))
            {
                return CreateResponseTask(request, HttpStatusCode.BadRequest, "在请求头中客户端公钥信息无法找到。");
            }
            string publicKey = request.Headers.GetValues(HttpHeaderNames.MesClientPublicKey).First();
            ...
            try
            {
                ...
                if (data == null)
                {
                    throw new MesException("服务器解析请求数据时发生异常。");
                }
